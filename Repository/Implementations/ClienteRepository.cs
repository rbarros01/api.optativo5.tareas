...
namespace Repository.Implementations
{
    public class ClienteRepository : IClienteRepository
    {
        ...
        public bool Add(ClienteDTO cliente)
        {
            try
            {
                if (conexionDB.Execute("INSERT INTO cliente (Id_banco, Nombre, Apellido, Documento, Direccion, email, Celular, Estado) VALUES (@IdBanco, @Nombre, @Apellido, @Documento, @Direccion, @Email, @Celular, @Estado)", cliente) > 0)
                    return true;
                else
                    return false;
            }
        ...
        public bool Update(ClienteDTO cliente)
        {
            ...
            try
            {
                if (conexionDB.Execute("UPDATE Cliente SET Id_banco = @IdBanco, Nombre = @Nombre, Apellido = @Apellido, Documento = @Documento, Direccion = @Direccion, Email = @Email, Celular = @Celular, Estado = @Estado WHERE Id = @Id", cliente) > 0)
                    return true;
                else
                    return false;
            }
